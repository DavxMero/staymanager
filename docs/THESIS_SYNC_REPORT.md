...
1. **Modul Billing DIGABUNG ke Modul Keuangan (2026-06-14)**: halaman `/billing` yang berdiri sendiri **dihapus** (kini hanya redirect ke `/financial?tab=invoices`). Manajemen invoice sekarang menjadi **tab "Invoices" di dalam /financial**. Alasan: /billing lama penuh UI palsu/mati (grafik revenue hardcoded, kartu aksi tak berfungsi, tabel duplikat) dan tidak ada di sidebar — membingungkan untuk peran finance. **Implikasi skripsi**: gabungkan narasi Modul Billing (Bab 4.2.11) ke dalam Modul Keuangan (Bab 4.2.8) sebagai sub-fungsi (Accounts Receivable), atau pertahankan sebagai sub-bab tetapi jelaskan bahwa Billing diakses sebagai tab Invoices di halaman Keuangan. **Gambar 4.8** (gambar-4-8-billing-invoice.png) sekarang menampilkan tab Invoices di /financial — perlu konfirmasi pemilik sebelum agent Word menggantinya. Tab Invoices menampilkan 10 invoice terbaru default + filter rentang tanggal (Load Range); tiap baris punya aksi View → halaman detail invoice (/billing/[id] masih ada).
2. **Modul Keuangan (/financial)** kini menjadi pusat keuangan tunggal dengan **4 tab: Overview · Invoices · Income History · Expenses**. Tambahan: tombol pintasan **"Billing & Invoices"** di header (membuka tab Invoices) dan kartu KPI keempat **"Pending Invoices"** (jumlah + nominal invoice belum dibayar, klik → tab Invoices); KPI menampilkan jumlah transaksi riil (bukan persentase pertumbuhan dummy); Recent Transactions punya tombol "View all" → tab Income History. Bug data invoice diperbaiki (kolom `total_amount`, sebelumnya membaca `amount` yang tidak ada sehingga semua nominal Rp 0). Relevan untuk Bab 4.2.8 dan bukti Aturan Emas #2 (shortcut). Komponen mati dihapus: ModernBillingDashboard, PaymentProcessing, AddInvoiceForm, AddInvoiceDialog, InvoiceViewer.
